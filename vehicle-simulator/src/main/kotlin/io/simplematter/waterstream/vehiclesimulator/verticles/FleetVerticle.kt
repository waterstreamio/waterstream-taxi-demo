...
class FleetVerticle : CoroutineVerticle() {
    ...
    override suspend fun start() {
        ...
        vertx.eventBus().consumer(FleetEvent.VehicleCreated, Handler<Message<JsonObject>> { event ->
            log.debug("vehicleCreated incoming event, reg size={}", vehiclesRegister.size())
            val v = extractVehicle(event)
            vehiclesRegister.add(vehicle = v, isNew = true)
            log.debug("vehicleCreated processed, reg size={}", vehiclesRegister.size())
        })
        ...
        vertx.eventBus().consumer(FleetEvent.VehicleRemoved, Handler<Message<JsonObject>> { event ->
            log.debug("vehicleRemoved incoming event, reg size={}", vehiclesRegister.size())
            val v = extractVehicle(event)
            if (v.visible)
                visibleVehiclesCount = visibleVehiclesCount - 1
            vehiclesRegister.remove(vehicle = v)
            log.debug("vehicleRemoved processed, reg size={}", vehiclesRegister.size())
        })
        ...
        vertx.eventBus().consumer(deployTick, Handler<Message<Int>> { _ ->
            ...
            if (missingVehicles > 0) {
                ...
                log.debug("Deploying {} vehicles or {} missing, current register size is {}", nVehicles, missingVehicles, vehiclesRegister.size())
                deployVehicleVerticles(serviceConfig, nVehicles)
            } else {
                log.debug("Not deploying new vehicles. Register size is {}", vehiclesRegister.size())
            }
        })

    }

    private val deployAttempts = AtomicInteger(0)
    ...
    private fun deployVehicleVerticles(config: VehicleSimulatorConfig, number: Int) {
        ...
        for (i in 0..number) {
            ...
            val attempt = deployAttempts.incrementAndGet()
            ...
            log.debug("deploy attempt {}, plate={}", attempt, vehicleVerticleConfig.plate)
...
//        return JsonUtils.fromVertxJson(value, Vehicle::class.java)
        ...
        val passengers = value.getInteger("passengers")
        val companyName = value.getString("companyName")
        val companyColor = value.getString("companyColor")
        val companyShape = value.getString("companyShape")
        val isHired = value.getBoolean("isHired")
        val id = value.getString("id")
        return Vehicle(plate, currentPoint, waypointPoint, updateTimestamp, speed, visible, passengers, companyName,
            companyColor, companyShape, isHired, id)
    }
}
